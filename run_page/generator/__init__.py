...
class Generator:
    ...
    def _fix_indoor_locations(activity_list):
        """Mark genuinely indoor activities so the frontend can render them
        without drawing a route.

        IMPORTANT: we only trust an *explicit* indoor/treadmill subtype that
        came from the data source itself (Garmin FIT "treadmill", Strava/Keep
        "VirtualRun", etc.). We deliberately do NOT try to infer "indoor"
        from an empty polyline or a small GPS bounding box: a perfectly real
        outdoor activity can have a temporarily missing or noisy GPS fix
        (e.g. dense tree canopy at a trailhead, a brief signal drop), and
        guessing "indoor" in that case used to make us splice in a *borrowed*
        route from a nearby previous activity — which silently displayed a
        completely wrong, unrelated location on the map for perfectly good
        outdoor data. If an activity really has no usable GPS track, it is
        far safer to just show no route than to show the wrong one.
        """
        ...
        for a in activity_list:
            ...
            if subtype in INDOOR_SUBTYPES:
                a["subtype"] = "indoor"
                # No fabricated route: leave the polyline empty. The
                # frontend already falls back to a location marker when
                # summary_polyline is empty/degenerate.
                a["summary_polyline"] = ""
